...
class SortedSetTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testCreateFromUnion()
    {
        ...
        SortedSet::createFromUnion('key4', array(
    ...
    public function testCreateFromIntersect()
    {
        $this->client->expects($this->once())->method('__call')->with('zinterstore', array(
            'key4', 3, 'key1', 'key2', 'key3', 'WEIGHTS', 1, 2, 1, 'AGGREGATE', 'MIN'));
        SortedSet::createFromIntersect('key4', array(
                new SortedSet($this->client, 'key1'),
                new SortedSet($this->client, 'key2'),
                new SortedSet($this->client, 'key3')
            ), array(1, 2),
    ...
    {
        $this->client->expects($this->once())->method('__call')->with('zrange', array('test_key', 0, -1))->will($this->returnValue(array()));
        iterator_to_array($this->type);
    }

    public function testGetRange()
    {
        $this->client->expects($this->once())->method('__call')->with('zrange', array('test_key', 5, -3))->will($this->returnValue(array()));
        $this->type->getRange(5, -3);
    }

    public function testGetRevRange()
    {
        $this->client->expects($this->once())->method('__call')->with('zrevrange', array('test_key', 5, -3))->will($this->returnValue(array()));
        $this->type->getRange(5, -3, SortedSet::REVERSE);
    }

    public function testGetRangeByScore()
    {
        $this->client->expects($this->once())->method('__call')->with('zrangebyscore', array('test_key', 5, -3, 'limit', 2, 5))->will($this->returnValue(array()));
        $this->type->getRange(5, -3, SortedSet::BY_SCORE, array(2, 5));
    }

    public function testGetRevRangeByScore()
    {
        $this->client->expects($this->once())->method('__call')->with('zrevrangebyscore', array('test_key', 5, -3, 'limit', 2, 5))->will($this->returnValue(array()));
        $this->type->getRange(5, -3, SortedSet::BY_SCORE | SortedSet::REVERSE, array(2, 5));
    }

    public function testGetRangeByScoreWithScores()
    {
        $this->client->expects($this->once())->method('__call')->with('zrangebyscore', array('test_key', 5, -3, 'withscores'))->will($this->returnValue(array()));
